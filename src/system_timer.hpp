...
constexpr uint32_t RAW_PIMR_VALUE = static_cast<uint32_t>(
    (static_cast<uint64_t>(ST_SLOW_CLOCK_HZ) * INTERRUPT_INTERVAL_MS) / 1000);
...
inline uint32_t read_pimr() { return volatile_read<uint32_t>(PIMR); }
...
static inline bool suppress_timer_output = false;
inline void set_suppressed(bool value) { suppress_timer_output = value; }
inline bool is_suppressed() { return suppress_timer_output; }
